...
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
  ...
  const generateBeams = useCallback(() => {
    const accentPalette = [228, 238, 248, 262, 276]
    const aspectRatios = [4, 6, 8, 5, 7]
    const beams = []
    ...
    for (let i = 0; i < beamsPerSide; i++) {
      ...
      const ratio = beamsPerSide <= 1 ? 0 : i / (beamsPerSide - 1)
      const delay = beamDelayMin + (beamDelayMax - beamDelayMin) * ratio
      beams.push({
        x,
        delay,
        hue: accentPalette[i % accentPalette.length] ?? 248,
        aspectRatio: aspectRatios[i % aspectRatios.length] ?? 6,
      })
    }
    return beams
